...
def add_trigger(person_list, nc, ges_len=5):
    if nc:
        nc_tmp = list()
        for item in nc:
                    ...
                    if 'relpos' in person:
                        for posp in person['relpos']:
                            for posn in nc['relpos']:
                                iou_hand = box_iou(torch.tensor(person[f'hand_class_{posn}']), torch.tensor(nc[f'hand_class_{posp}']))
                                if iou_hand > 0.95:
                                    # 手没有移动
                                    item['act'] = True
                                    if 'hand_class_left' in person:
...
def parase_now_person(person_list, nc):
    res = []
    if not nc:
        # nc 没有保存上一帧得到的结果，说明上一帧没有有效手势被检测到
        # 将当前帧的值赋值给nc
        nc = person_list
        for itm in nc:
            # 判断当前帧得到的结果有没有检测到手势
...
def main(args):
    ...
    for idx, img in enumerate(dataset):
        ...
        persons = persons
        nc = add_trigger(persons, nc)
        if args.show:
            for index, visualizer in enumerate(visualizers):
                if index >= len(visualizers) - 2:
