...
# from utils.facedataloaders import create_dataloader
from utils.landmark.dataloaders import create_dataloader
from utils.general import labels_to_class_weights, increment_path, labels_to_image_weights, init_seeds, \
    fitness, strip_optimizer, get_latest_run, check_dataset, check_file, check_git_status, check_img_size, \
    print_mutation, set_logging, intersect_dicts, check_suffix, check_yaml, colorstr, LOGGER
from utils.downloads import attempt_download, is_url
from utils.landmark.loss import compute_loss
from utils.landmark.plots import plot_labelsf, plot_resultsf, plot_evolution
...
def train(hyp, opt, device, tb_writer=None, wandb=None):
    ...
    if rank in [-1, 0]:
        ...
        if not opt.resume:
            ...
            if plots:
                plot_labelsf(labels, save_dir, loggers)
                if tb_writer:
    ...
    if rank in [-1, 0]:
        ...
        # Plots
        if plots:
            plot_resultsf(save_dir=save_dir)  # save as results.png
            if wandb:
...
def parse_opt(known=False):
    ...
    parser.add_argument('--weights', type=str, default=ROOT / 'yolov5n-lmk.pt', help='initial weights path')
    parser.add_argument('--cfg', type=str, default=ROOT / 'models/landmark/yolov5n-lmk.yaml',
                        help='model.yaml path')
    parser.add_argument('--data', type=str, default=ROOT / 'data/key_point.yaml', help='data.yaml path')
...
def main(opt, callbacks=Callbacks()):
    ...
    set_logging(str(opt.global_rank))
