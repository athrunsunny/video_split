...
def run(
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        with dt[2]:
            preds = non_max_suppression_face(preds,
                                             conf_thres,
                                             iou_thres,
                                             labels=lb)
        ...
        for si, pred in enumerate(preds):
            ...
            scale_boxes(im[si].shape[1:], predn[:, :4], shapes[si][0], shapes[si][1])  # native-space pred

            # Evaluate
            if nl:
                tbox = xywh2xyxy(labels[:, 1:5])  # target boxes
                # scale_boxes(im[si].shape[1:], tbox, shape, shapes[si][1])  # native-space labels
                scale_boxes(im[si].shape[1:], tbox, shapes[si][0], shapes[si][1])  # native-space labels
        ...
        if plots and batch_i < 3:
            plot_imagesf(im, targets, paths, save_dir / f'val_batch{batch_i}_labels.jpg', names)  # labels
            plot_imagesf(im, output_to_targetf(preds), paths, save_dir / f'val_batch{batch_i}_pred.jpg', names)  # pred
...
def parse_opt():
    ...
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / 'yolov5n-lmk.pt', help='model path(s)')
    ...
    parser.add_argument('--project', default=ROOT / 'runs/val-lmk', help='save to project/name')
