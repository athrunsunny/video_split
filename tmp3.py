...
def post_process_nc(nc):
    ...
    for person_info in nc:
        ...
        for pos in person_info['relpos']:
            ...
            # 处理hand_frame和record_hand
            if hand_frame and len(hand_frame) >= 5:
                if len(hand_frame) == len(record_hand):
                    # 通常情况这两个数据长度保持相等
                    if len(hand_frame) >= 5:
                        # 长度累计到5帧
                        pos_bool = list()
                        ...
                        if True in pos_bool:
                            person_info[f'out_{pos}'] = True
                            # 相关位置的参数要修改
                            # person_info[f'hand_bbox_{pos}'] = [0,0,0,0,0]
                            # person_info[f'hand_class_{pos}'] = ''
                        else:
                            # 判断全为N或大部分为N，基本用于判断全N
                            F, P, N, L, FN = cal_count(record_hand)
                            if N / 5. >= 0.8 or FN / 5. >= 0.8:
                                person_info[f'record_hand_{pos}'] = list()
                                person_info[f'hand_{pos}_frameid'] = list()
                    else:
                        # 长度不足5帧
                        # 模棱两可的检测结果（即分类概率小于一定阈值的结果）的判断，自适应增加检测次数
                        if record_hand[-1] == 'FN':
                            person_info[f'record_hand_{pos}'].pop(-1)
                            person_info[f'hand_{pos}_frameid'].pop(-1)
                elif len(hand_frame) > len(record_hand):
                    # 当帧数大于手势数时，暴力清理
                    person_info[f'hand_bbox_{pos}'] = [0,0,0,0,0]
                    person_info[f'hand_class_{pos}'] = ''
...
def cal_count(record_hand):
    F, P, N, L, FN = 0, 0, 0, 0, 0
    for ges in record_hand:
        if ges == 'F':
            F += 1
        elif ges == 'P':
            P += 1
...
def post_process_wc(wc):
    for person_info in wc:
        if 'record_hand' in person_info:
            person_info.pop('record_hand')
        for pos in person_info['relpos']:
            # hand_frame = list()
            record_hand = list()
            # if f'hand_{pos}_frameid' in person_info:
            ...
            # 只有在record_hand长度大于等于5时才进行处理，此时的处理是清理残存的列表元素
            if len(record_hand) >= 5:
                F, P, N, L, FN = cal_count(record_hand)
                if N / 5. >= 0.8 or FN / 5. >= 0.8:
                    person_info[f'record_hand_{pos}'] = list()
                    person_info[f'hand_{pos}_frameid'] = list()
    ...
    last_wc = list()
    now_wc = deepcopy(wc)
    for person in now_wc:
        tmp_bool = list()
        for pos in person['relpos']:
            if len(person[f'record_hand_{pos}']) == 0:
                # 空列表为False
                tmp_bool.append(False)
                # if f'out_{pos}' in person:
                person[f'hand_bbox_{pos}'] = [0, 0, 0, 0, 0]
                person[f'hand_class_{pos}'] = ''
                    # person.pop(f'out_{pos}')
            else:
                tmp_bool.append(True)
        if True in tmp_bool:
            last_wc.append(person)

    return last_wc


def cat_record_hand1(nc):
    # 合并record_hand
    for person in nc:
        record_hand = list()
        out_bool = list()
        ...
        for pos in person['relpos']:
            if f'out_{pos}' in person:
                if person[f'out_{pos}']:
                    ...
                    # 用于对其iou附近帧，防止多次显示
                    person[f'hand_{pos}_last_frameid'] = person[f'hand_{pos}_frameid'][-1]
                    # 满5删除
                    # person.pop(f'hand_{pos}_frameid')
                    # person.pop(f'record_hand_{pos}')
                    person[f'hand_{pos}_frameid'] = list()
                    person[f'record_hand_{pos}'] = list()
                    # 放到最后做
                    person.pop(f'out_{pos}')
            ...
            if posaft == pospre:
                ...
                if hand_iou > 0.80:
                    ...
                    if f'record_hand_{pospre}' not in personpre:
                        ...
                        # personpre[f'record_hand_{pospre}'] = ['TN'] #作为特殊条件判断，一开始没有检测到的时候插入
                    else:
                        # 当前手势在前一帧的待处理列表中
                        personpre[f'record_hand_{pospre}'].append('N')  # 同时需要在此加入判断列表中最后一个元素是否是TN的判断
...
def init_nc(nc,frame_id):
    positive_nc = list()
    for itm in nc:
        if 'relpos' in itm:
            pos_bool = list()
            for pos in itm['relpos']:
                if f'hand_class_{pos}' in itm:
...
def get_last_nc(current_frame, wipe_current_frame):
    for cur_frame in current_frame:
        cur_effect_area = cur_frame['effect_hand_area']
        for wcur_frame in wipe_current_frame:
            wcur_effect_area = wcur_frame['effect_hand_area']
            effect_area_iou = box_iou(torch.tensor([cur_effect_area]), torch.tensor([wcur_effect_area]))
            if effect_area_iou >= 0.8:
                # iou >0.8说明找到了与之前对其的帧
                for pos in cur_frame['relpos']:
                    if f'hand_{pos}_last_frameid' in wcur_frame:
                        last_frame_id = wcur_frame[f'hand_{pos}_last_frameid']
                        if f'hand_{pos}_frameid' in cur_frame:
                            cur_fid = cur_frame[f'hand_{pos}_frameid']
                            cur_record_hand = cur_frame[f'record_hand_{pos}']
                            new_cur_fid = list() # 保存大于上一次显示帧之后的帧数
                            new_cur_record_hand = list()
                            for itm,ges in zip(cur_fid,cur_record_hand):
                                if itm > last_frame_id:
                                    new_cur_fid.append(itm)
                                    new_cur_record_hand.append(ges)
                            cur_frame[f'hand_{pos}_frameid'] = new_cur_fid
                            cur_frame[f'record_hand_{pos}'] = new_cur_record_hand
                            wcur_frame.pop(f'hand_{pos}_last_frameid')

    return current_frame, wipe_current_frame
...
def add_trigger1(person_list, nc, frame_id, ges_len=5):
    ...
    for index, person in enumerate(person_list):
        # 先判断手框是否在person中
        # if 'hand' not in person: # len(person) <= 4
        if len(person) <= 4:
            continue
        tmp_person.append(person)
    new_person_list = tmp_person

    new_person_list = padding_zero(new_person_list)
    # nc是前一帧的检测结果
    # 如果nc为空，用当前帧的结果进行赋值
    if not nc:
        nc = new_person_list
    ...
    # 用于确定nc中的元素都包含正类别
    nc = init_nc(nc,frame_id)

    # new_person_list = padding_zero(new_person_list)
    # 可能还存在nc为None的情况，此时为清除不合理的结果
    if not nc:
        nc = new_person_list
        # 再初始化
        nc = init_nc(nc, frame_id)
    ...
    if not nc:
        return nc, []
    # 增加自动补齐功能，左右手没检测到的时候pad[0 for _ in range(5)]   # 先补齐402行
    # nc = padding_zero(nc)
        ...
        if 'act' not in personnow or not personnow['act']:
            ...
            if 'relpos' in personnow:
                ...
                for pos in personnow['relpos']:
                    if f'hand_class_{pos}' in personnow:
                ...
                if True in pos_bool:
                    ...
                    for posaft in personnow['relpos']:
                        if check_label(personnow[f'hand_class_{posaft}']):
                            # 当前帧检测到的手势是正例，则将正例记录到record_hand_{posaft}
                            if f'record_hand_{posaft}' not in personnow:
                                personnow[f'record_hand_{posaft}'] = [personnow[f'hand_class_{posaft}']]
                            else:
                                personnow[f'record_hand_{posaft}'].append(personnow[f'hand_class_{posaft}'])
                            ...
                            if f'record_hand_{posaft}' not in personnow:
                                # 当前手势不在前一帧的待处理列表中
                                personnow[f'record_hand_{posaft}'] = ['N']
                                # personnow[f'record_hand_{posaft}'] = ['TN'] #作为特殊条件判断，一开始没有检测到的时候插入
                            else:
                                # 当前手势在前一帧的待处理列表中
                                personnow[f'record_hand_{posaft}'].append('N')  # 同时需要在此加入判断列表中最后一个元素是否是TN的判断
    ...
    # print("wc",wc)
    # 此时获得前一次检测结果的最后一帧，对该结果附近iou较高的结果进行对其,裁撤在最后一帧之前的检测结果
    nc, wc = get_last_nc(nc, wc)
    # 对于已经输出的person进行处理
